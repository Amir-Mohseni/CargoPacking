...
public class Search {
    ...
    */
    ...
    static public boolean addBottomBackLeftForGreedy(int [][][] field, boolean[][][] piece, int id, int[] values) {
                ...
                for (int kField = field[iField][jField].length - 1; kField >= 0; kField--) { // for every x
                    ...
                    int[] coords = {
                        ...
                        if(isPlaceable(field, piece, coords)) {
                            ...
                            value += ParcelDatabase.getValue(id, values);
                            return true;
                        }
    ...
    */
    ...
    static public void addFromBottomBackLeft(int [][][] field, boolean[][][] piece, int id, int[] values) {
        for (int iField = field.length - 1; iField >= 0; iField--) { // for every z
            for (int jField = 0; jField < field[iField].length; jField++) { // for every y
                for (int kField = field[iField][jField].length - 1; kField >= 0; kField--) { // for every x
    ...
    */
    ...
    static public int[][][] generateFieldFromChromosomes(int[][] chromosomes, boolean[][][][][] database, int[] values) {
        value = 0;
        int[][][] field = new int[33][5][8];
        for(int i = 0; i < chromosomes[0].length; i++) {
            addFromBottomBackLeft(field, database[chromosomes[0][i]][chromosomes[1][i]], chromosomes[0][i] + 1, values);
        }
        return field;
    }
